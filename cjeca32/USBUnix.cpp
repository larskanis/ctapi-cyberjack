...
CBaseReader *CUSBUnix::BuildReaderObject() {
  ...
  if (dev->vendorId==AUSB_CYBERJACK_VENDOR_ID) {
    ...
    ptr=(int8_t *)(dev->productName);
    rv=strlen(dev->productName);

    switch(dev->productId) {
    case 0x400:
      if(rv>=18 && memcmp(ptr,"cyberJack e-com(a)",18)==0) {
	ptr[18]='\0';
	m_Reader = new CECAReader(m_Owner,this);
      }
      else if(rv>=24 && memcmp(ptr,"cyberJack e-com plus DUO",24)==0){
	ptr[24]='\0';
	m_Reader = new CECBReader(m_Owner,this);
      }
      else if(rv>=24 && memcmp(ptr,"cyberJack e-com plus BIO",24)==0){
	ptr[24]='\0';
	m_Reader = new CECBReader(m_Owner,this);
      }
      else if(rv>=25 && memcmp(ptr,"cyberJack e-com plus RFID",25)==0){
	ptr[25]='\0';
	m_Reader = new CECRReader(m_Owner,this);
      }
      else if(rv>=20 && memcmp(ptr,"cyberJack e-com plus",20)==0) {
	ptr[20]='\0';
	m_Reader = new CECPReader(m_Owner,this);
      }
      else if(rv>=17 && memcmp(ptr,"cyberJack Secoder",17)==0){
	ptr[17]='\0';
	m_Reader = new CSECReader(m_Owner,this);
      }
      else {
	DEBUGP(m_cDeviceName,DEBUG_MASK_COMMUNICATION_ERROR,
	       "Device [%s] ist not a known cyberJack 0x400, assuming e-com(a)", dev->productName);
	m_Reader = new CECAReader(m_Owner,this);
      }

      m_productString=strdup(dev->productName);
      m_pid=dev->productId;
      return m_Reader;

    case 0x401:
      if(rv>=18 && memcmp(ptr,"cyberJack e-com(f)",18)==0) {
	ptr[18]='\0';
	m_Reader = new CECFReader(m_Owner,this);
      }
      else if(rv>=19 && memcmp(ptr,"cyberJack e-com BIO",19)==0){
	ptr[19]='\0';
	m_Reader = new CEFBReader(m_Owner,this);
      }
      else {
	DEBUGP(m_cDeviceName,DEBUG_MASK_COMMUNICATION_ERROR,
               "Device [%s] ist not a known cyberJack 0x401, assuming e-com(f)", dev->productName);
	m_Reader=new CECFReader(m_Owner, this);
      }
    ...
    default:
      Debug.Out(m_cDeviceName,DEBUG_MASK_COMMUNICATION_ERROR,
		"Device is not a cyberjack 0x4xx",0,0);
      return NULL;
    }
  }
  else {
