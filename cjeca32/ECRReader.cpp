...
/*				if(InputLength!=8+lengthCHAT+lengthPIN+lengthCD)
		...
		case CJ_SUCCESS:
	      ...
	      InternalLength=(uint16_t)ResponseLen;
			memcpy(Output+4,&InternalLength,2);
            ...
            memcpy(Output+8,&InternalLength,2);
				if(ResponseLen>4+InternalLength)
				{
					lengthCAR=Output[10+InternalLength];
					if(ResponseLen>5+InternalLength+lengthCAR)
					{
						lengthCARprev=Output[11+InternalLength+lengthCAR];
						if(ResponseLen>7+InternalLength+lengthCAR+lengthCARprev)
						{
